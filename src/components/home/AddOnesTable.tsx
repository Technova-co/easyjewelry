...
import { addOnesTableData, AddOneStatus } from '@/data/addOnesTableData';
import SectionBanner from '@/components/shortCode/SectionBanner';
import InfoIcon from '../icons/InfoIcon';

const StatusIcon: React.FC<{ status: AddOneStatus }> = ({ status }) => {
  if (status === 'yes') {
    return (
    ...
    <section className="section-bottom-border">
      <div className="container">
        <div className="border-container section-spacing-lg">
          ...
          <SectionBanner
            variant="two"
            outlineButtonText="Add Ons"
        title="Go beyond the basics"
description="EasyJewelry's add-on modules let you offer loans, layaway plans, and schemes."
          />

          <div className="mt-10 md:mt-14 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {addOnesTableData.map((group) => (
              <div
                key={group.id}
...
AddOnesTable.displayName = 'AddOnesTable';

export default AddOnesTable;